...
"blandr" is a package to carry out Bland Altman analyses (also known as a Tukey mean-difference plot) in R. This package was started in 2015 as existing Bland-Altman R functions did not calculate confidence intervals: my belief is that drawing Bland-Altman plots, confidence intervals should be considered the gold-standard. The blandr fills this need!

It further generates plotting data to allow easy generation of charts using ggplot, as well as R's in-built drawing capabilities.
...
To load the sample PEFR data use the following command:
```
load(file="Data/bland.altman.1986.PEFR.rda")
load(file="Data/giavarina.2015.rda")
```

This will give you four data sets for comparison: WrightFirst, WrightSecond, MiniWrightFirst, and MiniWrightSecond. We can calculate agreement between WrightFirst and MiniWrightFirst.
...
```{r}
blandr.statistics ( WrightFirst , MiniWrightFirst , sig.level=0.95 )
```
...
```{r}
stats.data <- blandr.statistics ( WrightFirst , MiniWrightFirst , sig.level=0.95 )
summary(stats.data)
stats.data
```

You can then take the individual elements of the statistics output by appending a number in square brackets to the end when callign the list, e.g.:
...
```{r}
blandr.display ( WrightFirst , MiniWrightFirst , sig.level=0.95 )
```
...
blandr.draw( WrightFirst , MiniWrightFirst )
```
...
```{r, fig.width=6, fig.height=5}
blandr.draw( WrightFirst , MiniWrightFirst , ciDisplay = FALSE , ciShading = FALSE )
```
...
```{r, fig.width=6, fig.height=5}
vignette.chart <- blandr.draw( WrightFirst , MiniWrightFirst )
vignette.chart
...
```{r, fig.width=6, fig.height=5}
blandr.draw( WrightFirst , MiniWrightFirst , ciDisplay = FALSE )
```
...
```{r, fig.width=6, fig.height=5}
vignette.chart <- blandr.draw( WrightFirst , MiniWrightFirst , ciDisplay = FALSE )
vignette.chart
```
